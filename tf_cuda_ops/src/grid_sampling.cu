...
__global__ void grid_sampling_gpu_kernel(int batch_size, int input_point_num,
	...
	if (point_id < input_point_num) {
        ...
        int grid_coor_w = (int)floor(input_coors[point_id*3 + 0] / res_x);
        int grid_coor_l = (int)floor(input_coors[point_id*3 + 1] / res_y);
        int grid_coor_h = (int)floor(input_coors[point_id*3 + 2] / res_z);
        
        // int grid_buffer_idx = batch_id * grid_size + grid_coor_w * grid_l * grid_h + grid_coor_l * grid_h + grid_coor_h;
        long long grid_buffer_idx = batch_id * grid_size + grid_coor_h * grid_l * grid_w + grid_coor_l * grid_w + grid_coor_w;

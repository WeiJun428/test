...
public:
    ...
    void Compute(OpKernelContext* context) override {
        ...
        int grid_w = (int)ceil(dimension[0] / resolution[0]) + 1;
        int grid_l = (int)ceil(dimension[1] / resolution[1]) + 1;
        int grid_h = (int)ceil(dimension[2] / resolution[2]) + 1;
        ...
        // OP_REQUIRES(context, input_coors.dim_size(0)== (input_accu_list_ptr_host[batch_size-1] + input_num_list_ptr_host[batch_size-1]),
        //             errors::InvalidArgument("GridSamplingOp expects input_coors dim 0 to have same size as total number of points in input_num_list. [%d] %= [%d] \n",
        //             input_coors.dim_size(0),
        //             (input_accu_list_ptr_host[batch_size-1] + input_num_list_ptr_host[batch_size-1])));
        if (input_coors.dim_size(0) != (input_accu_list_ptr_host[batch_size-1] + input_num_list_ptr_host[batch_size-1])){
            printf("GridSamplingOp expects input_coors dim 0 to have same size as total number of points in input_num_list. [%d] not equal [%d] \n",
                    (int)input_coors.dim_size(0), (input_accu_list_ptr_host[batch_size-1] + input_num_list_ptr_host[batch_size-1]) );
        }
...
//         int input_point_num = input_coors.dim_size(0);
//         int batch_size = input_num_list.dim_size(0);
//         int grid_w = (int)ceil(dimension[0] / resolution);
//         int grid_l = (int)ceil(dimension[1] / resolution);
//         int grid_h = (int)ceil(dimension[2] / resolution);
//         if (INT_MAX / grid_h / grid_l / grid_w < batch_size){
//             printf("GridSamplingOp ERROR: size of grid buffer %d x [%d x %d x %d] exceeds INT32 range: %d.\n",
// 	                batch_size, grid_w, grid_l, grid_h, INT_MAX);}

...
REGISTER_OP("GetRoiBboxOp")
    ...
    .Output("roi_cls_conf: int32")
    .Output("roi_det_conf: int32")
    .Output("roi_cls: int32")
    ...
    .SetShapeFn([](InferenceContext* c){
        ...
        ShapeHandle roi_cls_conf_shape = c->MakeShape({npoint});
        ShapeHandle roi_det_conf_shape = c->MakeShape({npoint});
        ShapeHandle roi_cls_shape = c->MakeShape({npoint});
        c->set_output(0, roi_bbox_shape);
        c->set_output(1, roi_cls_conf_shape);
        c->set_output(2, roi_det_conf_shape);
        c->set_output(2, roi_cls_shape);
...
public:
    explicit GetRoiBboxOp(OpKernelConstruction* context): OpKernel(context) {
        ...
        OP_REQUIRES_OK(context, context->GetAttr("det_thres", &det_thres));
    ...
    void Compute(OpKernelContext* context) override {
        ...
        Tensor* roi_cls_conf = nullptr;
        auto roi_cls_conf_shape = TensorShape({npoint});
        OP_REQUIRES_OK(context, context->allocate_output(1, roi_cls_conf_shape, &roi_cls_conf));
        int* roi_cls_conf_ptr = roi_cls_conf->template flat<int>().data();
        cudaMemset(roi_cls_conf_ptr, 0, npoint * sizeof(int));

        Tensor* roi_det_conf = nullptr;
        auto roi_det_conf_shape = TensorShape({npoint});
        OP_REQUIRES_OK(context, context->allocate_output(2, roi_det_conf_shape, &roi_det_conf));
        int* roi_det_conf_ptr = roi_det_conf->template flat<int>().data();
        cudaMemset(roi_det_conf_ptr, 0, npoint * sizeof(int));

        Tensor* roi_cls = nullptr;
        auto roi_cls_shape = TensorShape({npoint});
        OP_REQUIRES_OK(context, context->allocate_output(3, roi_cls_shape, &roi_cls));
        int* roi_cls_ptr = roi_cls->template flat<int>().data();
        cudaMemset(roi_cls_ptr, 0, npoint * sizeof(int));

...
//            printf("%d=%d+%d, %d\n", center_accu_list_ptr_host[b], center_accu_list_ptr_host[b-1], center_num_list_ptr_host[b-1], center_num_list_ptr_host[b]);
        ...
        if ((int)input_coors.dim_size(0) != (input_accu_list_ptr_host[batch_size-1] + input_num_list_ptr_host[batch_size-1])){
            printf("VoxelSamplingIdxBinary expects input_coors dim 0 to have same size as total number of points in input_num_list. [%d] not equal [%d] \n",
                    (int)input_coors.dim_size(0), (input_accu_list_ptr_host[batch_size-1] + input_num_list_ptr_host[batch_size-1]) );
        }
        
        if (center_num != (center_accu_list_ptr_host[batch_size-1] + center_num_list_ptr_host[batch_size-1])){
            printf("VoxelSamplingIdxBinary expects center_coors dim 0 to have same size as total number of points in center_num_list. [%d] not equal [%d] \n",
                    (int)center_coors.dim_size(0), (center_accu_list_ptr_host[batch_size-1] + center_num_list_ptr_host[batch_size-1]) );
        }

        
        // OP_REQUIRES(context, center_num != (center_accu_list_ptr_host[batch_size-1] + center_num_list_ptr_host[batch_size-1]),
        //             errors::InvalidArgument("VoxelSamplingIdxBinary expects center_coors dim 0 to have same size as total number of points in center_num_list.\n"));
